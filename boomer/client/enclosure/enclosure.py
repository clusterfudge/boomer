...
from boomer.client.enclosure.arduino import EnclosureArduino
from boomer.client.enclosure.eyes import EnclosureEyes
from boomer.client.enclosure.mouth import EnclosureMouth
from boomer.client.enclosure.weather import EnclosureWeather
...
class EnclosureReader(Thread):
    ...
    E.g. Boomer Stop Feature
        #. Arduino sends a Stop command after a button press on a Boomer unit
        #. ``EnclosureReader`` captures the Stop command
        #. Notify all Boomer Core processes (e.g. skills) to be stopped
    ...
    def process(self, data):
        ...
        if "boomer.stop" in data:
            self.client.emit(Message("boomer.stop"))
            kill(['mimic'])  # TODO - Refactoring in favor of Boomer Stop
...
class EnclosureWriter(Thread):
    ...
    Writes data to Serial port.
        #. Enqueues all commands received from Boomer enclosures
           implementation
        #. Process them on the received order by writing on the Serial port

    E.g. Displaying a text on Boomer's Mouth
...
class Enclosure:
    """
    Serves as a communication interface between Arduino and Boomer Core.
    ...
    def __register_events(self):
        self.client.on('boomer.paired', self.__update_events)
